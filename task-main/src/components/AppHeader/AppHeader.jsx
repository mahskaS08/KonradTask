...
function goToNode(propertyLookupTree, searchText) {
  let currNode = propertyLookupTree;

  // Traverse
  for (const character of searchText) {
    if (currNode[character]) {
      currNode = currNode[character];
    } else {
      return undefined;
    }
  }
 ...
 */
function collectAllMatches(treeNode) {
  let allMatches = [];

  if (treeNode.match) {
    allMatches.push(treeNode.match);
  }

  for (const childKey in treeNode) {
    if (childKey !== "match") {
      allMatches = allMatches.concat(collectAllMatches(treeNode[childKey]));
    }
  }
...
export function findMatches(propertyLookupTree, searchText) {
  if (!searchText) return []; // Return an empty array if searchText is empty (New handling for edge case)

  const node = goToNode(propertyLookupTree, searchText);

  if (!node) return []; // Return an empty array if no matching node is found (New handling for missing nodes)
